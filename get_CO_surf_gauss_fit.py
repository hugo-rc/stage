...
class maxima_finder(Tk):
    ...
    def compute_maxima(self):
        ...
        if len(self.mouse)>0:
            ...
            for x in range(len(masked_im)):
                ...
                y=np.argmax(column)
                if column[y]!=0:
                    popt,pcov=curve_fit(self.gaussian,[i for i in range(len(column))], column, bounds=([-np.inf,y-20,-np.inf],[np.inf,y+20,np.inf]))
                    a,y0,sigma=popt
                    self.pos_maxima.append((x,y0))
                    self.max_value.append((x,a))
                    y0=int(y0)
    ...
    def next_chan(self):
        ...
        if self.n<self.nv-1:
            ...
            self.storage_max_value.append(copy(self.max_value))            
            self.n+=1
            self.reset()
...
class storage():
    ...
    def __init__(self, CO, PA, xs, ys, pos_maxima, max_value, obj, ni, nm, nf, window):
        self.CO=CO
...
if __name__ == "__main__":
    path="/home/hugo/Documents/Stage/selection_objets/"                         ######### /!\ TO BE MODIFIED FOR EACH OBJECT
    obj="HD163296" #studied object                                              ######### /!\ TO BE MODIFIED FOR EACH OBJECT
    continuum_fits=path+"HD163296/Itziar/HD163296_continuum.fits"                   ######### /!\ TO BE MODIFIED FOR EACH OBJECT
    fits_name = path+"HD163296/Itziar/HD163296_CO3-2.fits.gz"                   ######### /!\ TO BE MODIFIED FOR EACH OBJECT
    ...
    for e in ext:
        ...
        Max_f.mainloop()    
        data=storage(CO,PA_f.PA,xs,ys,Max_f.storage_pos_max, Max_f.storage_max_value,obj, ni, nm, nf, window)
        file_name=fits_name+e+'.gaussian.co_surf'
