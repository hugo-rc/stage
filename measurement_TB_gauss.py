...
max_sup_back=np.array(data_sup_back.max_value)
max_sup_front=np.array(data_sup_front.max_value)
max_inf_back=np.array(data_inf_back.max_value)
max_inf_front=np.array(data_inf_front.max_value)
    ...
    if i <= 18 or i >= 36 : # removing the chans where the arms were too vertical               ######### /!\ TO BE MODIFIED FOR EACH OBJECT
        ...
        try:
            ...
            for k in range(len(x)):
                ...
                T_sup_f=np.append(T_sup_f,flux_to_Tbrigh(max_sup_front[i][k][1],wl,BMAJ,BMIN))
                T_sup_b=np.append(T_sup_b,flux_to_Tbrigh(max_sup_back[i][k][1],wl,BMAJ,BMIN))
        ...
        try:
            ...
            for k in range(len(x)):
                ...
                T_inf_f=np.append(T_inf_f,flux_to_Tbrigh(max_inf_front[i][k][1],wl,BMAJ,BMIN))
                T_inf_b=np.append(T_inf_b,flux_to_Tbrigh(max_inf_back[i][k][1],wl,BMAJ,BMIN))
...
delta_sup=1 #                                               ######### /!\ TO BE MODIFIED FOR EACH OBJECT
delta_inf=1                                             ######### /!\ TO BE MODIFIED FOR EACH OBJECT            
idx_sup_front_max=[]
idx_sup_back_max=[]
idx_inf_front_max=[]
idx_inf_back_max=[]
...
r_sup_flat=copy(r_sup_flat[idx_sup_front_max])
Tb_sup_front_flat=copy(Tb_sup_front_flat[idx_sup_front_max])
r_inf_flat=copy(r_inf_flat[idx_inf_front_max])
Tb_sup_back_flat=copy(Tb_sup_back_flat[idx_sup_back_max])
Tb_inf_front_flat=copy(Tb_inf_front_flat[idx_inf_front_max])
Tb_inf_back_flat=copy(Tb_inf_back_flat[idx_inf_back_max])
